...
const start = 0;
const limit = 10;
const eachPageUsers = data.slice();
// const cycles = length / limit; //10/10 = 1
const cycles = Math.ceil(length / limit);

console.log(length, start, limit, cycles);

//loadRecords() - Load no of records which shoulf be visible to user.
...
// let output = `<button onclick=first()>First</button> <button onclick=previous()><i class="fa-solid fa-angles-left"></i> </button>`;
function loadButtons(cycles) {
  let output = `<button onclick=previous()><i class="fa-solid fa-angles-left"></i> </button>`;
  for (let i = 0; i < cycles; i++) {
    output += `<button id=${i} onclick=paginate(${i})>${i + 1}</button>`;
  }

  output += `<button onclick=next()><i class="fa-solid fa-angles-right"></i> </button>`;
  controls.innerHTML = output;

  //Set current class of first button.
...
function paginate(id) {
  ...
  //Remove current class from previously active button.
  const ele = document.getElementsByClassName("current");
  ele[0].classList.remove("current");

  //Assign current class to currently active button.
  document.getElementById(id).classList.add("current");

  loadRecords(start, end);
}

//Function to set controls when previous button is clicked
function previous() {
  //Get currently active button.
  const ele = document.getElementsByClassName("current");

  const current_id = parseInt(ele[0].id);

  //Set previous button as active if current button id > 0.
  current_id - 1 < 0
    ? alert("No More Page to Display")
    : paginate(current_id - 1);
}

//Function to set controls when next button is clicked.
function next() {
  //Get currently active button.
  const ele = document.getElementsByClassName("current");
  ...
  //Set next button as active if current button id < 10.
  //   current_id + 1 >= 10 ? paginate(0) : paginate(current_id + 1);
  current_id + 1 >= 10
    ? alert("No More Page to Display")
    : paginate(current_id + 1);
}

//Function to set controls when first button is clicked
function first() {
  paginate(0);
}
